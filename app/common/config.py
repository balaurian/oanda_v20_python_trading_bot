...
class Config(object):
    ...
    def load(self, path):
        ...
        try:
            with open(os.path.expanduser(path)) as f:
                #y = yaml.load(f) #generates warning YAMLLoadWarning: calling yaml.load() without Loader=... is deprecated | fix https://github.com/yaml/pyyaml/wiki/PyYAML-yaml.load(input)-Deprecation
    ...
    def create_context(self):
        ...
        #print ('create_context @ config.py', ctx)
        return ctx
...
def make_config_instance(path):
    ...
    config.validate()
    #print ('config @ config.py',config)
    #print ('type config @ config.py', type(config))
    return config
...
#def read_yaml_config():
    #care sa trigaruiasca make_config_instance(path)
    #pentru ca returneaza un config object
    #ca sa  scap de args,
    #citesc yaml default=default_config_path()
    #yaml.username type=make_config_instance
    #yaml.active_account
